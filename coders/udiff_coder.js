...
const SearchReplace = require('./search_replace');
...
class UnifiedDiffCoder extends Coder {
    ...
    apply_edits(edits) {
     ...
     flexi_just_search_and_replace(texts) {
        let strategies = [
     ...
     make_new_lines_explicit(content, hunk) {
        ...
        let new_hunk = difflib.unified_diff(new_before, after, {n: Math.max(new_before.length, after.length)});
        new_hunk = new_hunk.slice(3);

        return new_hunk;
    }

     cleanup_pure_whitespace_lines(lines) {
     ...
     normalize_hunk(hunk) {
        ...
        let diff = difflib.unified_diff(before, after, {n: Math.max(before.length, after.length)});
        diff = diff.slice(3);
        return diff;
    }

     directly_apply_hunk(content, hunk) {
        let [before, after] = hunk_to_before_after(hunk);
     ...
     apply_partial_hunk(content, preceding_context, changes, following_context) {
        ...
        while (line_num < lines.length) {
            while (line_num < lines.length) {
                ...
                if (line.startsWith("```diff")) {
                    let [line_num, these_edits] = process_fenced_block(lines, line_num + 1);
     ...
     process_fenced_block(lines, start_line_num) {
        for (let line_num = start_line_num; line_num < lines.length; line_num++) {
            let line = lines[line_num];
            if (line.startsWith("```")) {
                break;
            }
        ...
        if (block[0].startsWith("--- ") && block[1].startsWith("+++ ")) {
            ...
            let fname = block[1].slice(4).trim();
            block = block.slice(2);
        } else {
            let fname = null;
        }
     ...
     hunk_to_before_after(hunk, lines=false) {
        let before = [];
        let after = [];
        let op = " ";
